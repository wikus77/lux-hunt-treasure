...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func configureWebView() -> Bool {
        ...
        // 🔧 FIX 03/02/2026: TRANSPARENT WebView + BLACK native background
        // CRITICAL: Web gradient is the authoritative background.
        // Native layer = BLACK, WKWebView = TRANSPARENT → web gradient shows through.
        // This eliminates white safe-area forever.
        let bgBlack = UIColor(red: 10/255, green: 11/255, blue: 15/255, alpha: 1.0) // #0a0b0f
        webView.isOpaque = false // TRANSPARENT - let web gradient show
        webView.backgroundColor = .clear
        webView.scrollView.backgroundColor = .clear
        
        // Set root view background to BLACK (fallback if web not loaded)
        if let rootView = webView.superview {
            rootView.backgroundColor = bgBlack
        }
        
        // Set window background to BLACK
        window?.backgroundColor = bgBlack
