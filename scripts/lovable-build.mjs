...
function run(cmd) {
  ...
  try {
    ...
    writeFileSync(logFile, (out || ""), { flag: "a" });
    return 0;
  } catch (e) {
    ...
    writeFileSync(logFile, (out + err), { flag: "a" });
    return e?.status ?? 1;
  }
}

console.log("=== LOVABLE BUILD START ===");
console.log("Node/Env check:");
run("node -v");
run("npm -v || true");
run("pnpm -v || true");
run("which node || true");
...
console.log("\n=== INSTALL (LOCKED) ===");
let code = run("pnpm install --frozen-lockfile");
if (code !== 0) process.exit(code);

console.log("\n=== PREBUILD (GUARD) ===");
code = run("pnpm run -s prebuild");
if (code !== 0) process.exit(code);

console.log("\n=== BUILD (VITE) ===");
code = run("pnpm run -s build");
if (code !== 0) {
  console.error("\n❌ BUILD FAILED — see log:", logFile);
  // estrai prima riga errore visibile in console
...
console.log("\n=== DIST CHECK ===");
code = run("ls -la dist | sed -n '1,120p'");
if (code !== 0) process.exit(code);

console.log("\n✅ BUILD OK — log:", logFile);
process.exit(0);
