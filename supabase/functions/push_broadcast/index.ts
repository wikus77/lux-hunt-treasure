...
function cors(req: Request, init: ResponseInit = {}, body?: BodyInit) {
  ...
  h.set("access-control-allow-headers", "authorization, content-type, x-client-info, X-M1-Dropper-Version");
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const versionHeader = req.headers.get('X-M1-Dropper-Version');
    if (versionHeader !== 'v1') {
      const requestId = crypto.randomUUID();
      return cors(req, { status: 400 }, JSON.stringify({
