...
-- Progressive BUZZ MAPPA™ System - Database Schema Update

-- Create user_buzz_map_counter table if it doesn't exist
...
-- Enable RLS on user_buzz_map_counter
ALTER TABLE public.user_buzz_map_counter ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for user_buzz_map_counter
CREATE POLICY "Users can view their own buzz map counter" 
ON public.user_buzz_map_counter FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own buzz map counter" 
ON public.user_buzz_map_counter FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own buzz map counter" 
ON public.user_buzz_map_counter FOR UPDATE 
USING (auth.uid() = user_id);

-- Create updated_at trigger for user_buzz_map_counter
CREATE TRIGGER update_user_buzz_map_counter_updated_at 
BEFORE UPDATE ON public.user_buzz_map_counter 
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- Update buzz_map_actions table to include generation_number for tracking
ALTER TABLE public.buzz_map_actions 
ADD COLUMN IF NOT EXISTS generation_number INTEGER DEFAULT 0;

-- Create index for performance on buzz_map_actions
CREATE INDEX IF NOT EXISTS idx_buzz_map_actions_user_created 
ON public.buzz_map_actions(user_id, created_at DESC);

-- Create index for performance on user_buzz_map_counter
CREATE INDEX IF NOT EXISTS idx_user_buzz_map_counter_user_date 
ON public.user_buzz_map_counter(user_id, date DESC);
...
-- Create function to get current generation count for a user
