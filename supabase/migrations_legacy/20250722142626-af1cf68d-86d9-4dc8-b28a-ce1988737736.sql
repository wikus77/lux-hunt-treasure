...
-- Update legal documents constraint and insert complete legal content

-- Drop existing check constraint
ALTER TABLE public.legal_documents DROP CONSTRAINT IF EXISTS legal_documents_type_check;

-- Add new check constraint with all required types
ALTER TABLE public.legal_documents ADD CONSTRAINT legal_documents_type_check 
CHECK (type = ANY (ARRAY['terms_of_service'::text, 'privacy_policy'::text, 'cookie_policy'::text, 'giveaway_rules'::text, 'disclaimer'::text, 'safecreative'::text]));

-- Delete and recreate all legal documents with complete content
DELETE FROM public.legal_documents WHERE type IN ('terms_of_service', 'privacy_policy', 'cookie_policy', 'disclaimer', 'safecreative');
