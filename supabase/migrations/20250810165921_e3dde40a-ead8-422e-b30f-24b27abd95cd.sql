...
-- Hardening RLS, cleanup duplicati robusto, vincoli QR e report (v3)
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='api_rate_limits' AND policyname='arl_mutate_srv'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='blocked_ips' AND policyname='bip_mutate_srv'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='qr_redemption_logs' AND policyname='qrrl_insert_user_or_srv'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='qr_redemption_logs' AND policyname='qrrl_select_own'
  ) THEN
...
-- 5) Cleanup duplicati robusto: tenta created_at poi fallback su id
DO $$
BEGIN
  BEGIN
