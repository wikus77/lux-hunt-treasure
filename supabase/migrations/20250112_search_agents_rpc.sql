-- Search agents RPC function for TRON Battle
...
BEGIN
  ...
  ORDER BY 
    ...
    CASE 
      WHEN LOWER(p.username) = LOWER(search_term) THEN 0
      WHEN LOWER(p.agent_code) = LOWER(search_term) THEN 1
      WHEN p.username ILIKE search_term || '%' THEN 2
      WHEN p.agent_code ILIKE search_term || '%' THEN 3
      ELSE 4
    END,
    p.username
  LIMIT max_results;
