-- Update legal_documents with enhanced compliance clauses (without updated_at column)
UPDATE public.legal_documents 
SET 
  content_md = '# REGOLAMENTO UFFICIALE DEL GIOCO – M1SSION™

**Art. 1 – Oggetto del Gioco**

M1SSION™ è un''applicazione interattiva ideata da NIYVORA KFT™, con sede legale in Budapest, Ungheria, che propone un''esperienza di gioco a tema investigativo basata esclusivamente sull''abilità. L''obiettivo degli utenti è quello di localizzare e conquistare premi reali attraverso un sistema basato su enigmi, missioni, mappe e indizi geolocalizzati. Il gioco è accessibile esclusivamente tramite l''app M1SSION™ disponibile in versione web/PWA.

**🎯 CLAUSOLA ANTI-GAMBLING - SKILL-BASED ONLY**

M1SSION™ non assegna premi tramite meccanismi casuali o aleatori. Tutti i premi sono ottenuti esclusivamente attraverso abilità, deduzione e azione da parte dell''utente. Non esistono elementi di fortuna, sorteggio, o RNG (Random Number Generation). La ruota giornaliera presente nell''app è un elemento puramente cosmetico e decorativo che non influenza l''assegnazione di premi.
...
**Art. 5 – Modalità di Assegnazione dei Premi SKILL-BASED**

Il premio viene assegnato esclusivamente in base ad abilità logica, deduzione e inserimento coordinate:
...
Il pagamento di abbonamenti o BUZZ non garantisce né favorisce la vincita. La vittoria dipende unicamente dall''abilità dell''utente.
...
© 2025 – M1SSION™ – ALL RIGHTS RESERVED – NIYVORA KFT™',
  version = '1.1',
  published_at = now()
WHERE type = 'game_rules' AND is_active = true;