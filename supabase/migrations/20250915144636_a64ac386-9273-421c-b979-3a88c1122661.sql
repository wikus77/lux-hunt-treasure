...
-- 6) VERIFICHE RAPIDE (non interrompono se vuote)
--    (A) esistenza funzioni
select 'HAS_create_free_subscription' as check, exists(
  select 1 from pg_proc where proname='create_free_subscription'
) as ok;

select 'HAS_mark_choose_plan_seen' as check, exists(
  select 1 from pg_proc where proname='mark_choose_plan_seen'
) as ok;

--    (B) colonne/indici presenti
select 'profiles.choose_plan_seen' as check,
       (select count(*)=1 from information_schema.columns
         where table_schema='public' and table_name='profiles' and column_name='choose_plan_seen') as ok;

select 'idx_unique_active_sub_per_user' as check,
       exists(select 1 from pg_indexes where schemaname='public' and indexname='idx_unique_active_sub_per_user') as ok;