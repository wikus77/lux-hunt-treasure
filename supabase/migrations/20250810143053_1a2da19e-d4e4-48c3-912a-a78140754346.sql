...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY ub_select_own ON public.user_balances FOR SELECT TO authenticated USING (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='user_balances' AND policyname='ub_update_own'
  ) THEN
    CREATE POLICY ub_update_own ON public.user_balances FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY ucr_select_own ON public.user_custom_rewards FOR SELECT TO authenticated USING (user_id = auth.uid());
  END IF;
END $$;
...
-- FK su qr_redemption_logs -> qr_codes (NOT VALID per evitare blocchi)
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='qr_redemption_logs') THEN
    IF NOT EXISTS (
      ...
      ALTER TABLE public.qr_redemption_logs
        ...
        FOREIGN KEY (qr_code_id) REFERENCES public.qr_codes(id) ON DELETE CASCADE NOT VALID;
    END IF;
  END IF;
END $$;

-- UNIQUE index solo se non ci sono duplicati
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='qr_redemption_logs') THEN
    IF NOT EXISTS (
      SELECT 1 FROM (
        SELECT qr_code_id, user_id, COUNT(*) AS c
