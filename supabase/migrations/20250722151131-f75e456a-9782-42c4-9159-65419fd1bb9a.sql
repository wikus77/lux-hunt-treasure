-- First, drop the old constraint and add a new one that includes game_rules
ALTER TABLE public.legal_documents DROP CONSTRAINT IF EXISTS legal_documents_type_check;

-- Add the updated constraint that includes game_rules
ALTER TABLE public.legal_documents ADD CONSTRAINT legal_documents_type_check 
CHECK (type IN ('terms_of_service', 'privacy_policy', 'cookie_policy', 'disclaimer', 'safecreative', 'game_rules'));

-- Now insert the game rules document
