...
-- QR Codes schema, RLS and RPC for free reward flow (discovery -> claim -> credit)
...
-- 3) Policies
-- Show only discovered codes to authenticated users
CREATE POLICY IF NOT EXISTS "qr: authed read discovered only"
...
-- Admin full access to qr_codes
CREATE POLICY IF NOT EXISTS "qr: admin full"
...
-- Discoveries: owner can read
CREATE POLICY IF NOT EXISTS "discoveries: owner read"
ON public.qr_code_discoveries FOR SELECT TO authenticated
USING (user_id = auth.uid());

-- Discoveries: owner can insert
CREATE POLICY IF NOT EXISTS "discoveries: insert self"
ON public.qr_code_discoveries FOR INSERT TO authenticated
WITH CHECK (user_id = auth.uid());

-- Claims: owner can read
CREATE POLICY IF NOT EXISTS "claims: owner read"
ON public.qr_code_claims FOR SELECT TO authenticated
USING (user_id = auth.uid());

-- Claims: owner can insert
CREATE POLICY IF NOT EXISTS "claims: insert self"
ON public.qr_code_claims FOR INSERT TO authenticated
WITH CHECK (user_id = auth.uid());

-- 4) RPC qr_redeem(code text)
