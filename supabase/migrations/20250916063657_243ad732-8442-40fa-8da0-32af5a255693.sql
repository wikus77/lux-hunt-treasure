-- ===========================================
-- M1SSION™ FREE PLAN – DB SAFETY PACK
...
-- 4) RPC: create_free_subscription (idempotente, idempotent insert-or-ignore)
...
-- 6) CHECK rapido (stampa stato)
with s as (
  select auth.uid() as me
)
select
  (select count(*) from public.subscriptions where tier='free' and status='active') as free_active,
  (select count(*) from public.subscriptions where status='active') as active_total,
  (select choose_plan_seen from public.profiles p join s on p.id = s.me limit 1) as me_choose_plan_seen
;