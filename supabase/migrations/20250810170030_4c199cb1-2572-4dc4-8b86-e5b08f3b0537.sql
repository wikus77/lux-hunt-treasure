...
-- Hardening RLS, cleanup duplicati robusto, vincoli QR e policy (v4)
-- =========================================================

SET search_path = public;

-- 1) Funzioni critiche: forza search_path
DO $$
BEGIN
  BEGIN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='user_balances' AND policyname='ub_select_own') THEN
    EXECUTE 'create policy ub_select_own on public.user_balances for select to authenticated using (user_id = auth.uid())';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='user_balances' AND policyname='ub_update_own') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='user_custom_rewards' AND policyname='ucr_select_own') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='api_rate_limits' AND policyname='arl_select_admin') THEN
    EXECUTE 'create policy arl_select_admin on public.api_rate_limits for select to authenticated using (is_admin_secure())';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='api_rate_limits' AND policyname='arl_mutate_srv') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='blocked_ips' AND policyname='bip_select_admin') THEN
    EXECUTE 'create policy bip_select_admin on public.blocked_ips for select to authenticated using (is_admin_secure())';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='blocked_ips' AND policyname='bip_mutate_srv') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='qr_redemption_logs' AND policyname='qrrl_insert_user_or_srv') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='qr_redemption_logs' AND policyname='qrrl_select_own') THEN
...
-- 4) Indice su qr_codes
CREATE INDEX IF NOT EXISTS idx_qr_codes_code ON public.qr_codes(code);

-- 5) FK NOT VALID se mancante
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conrelid = 'public.qr_redemption_logs'::regclass AND conname = 'qr_redemption_logs_qr_code_fk'
  ) THEN
...
-- 6) Cleanup duplicati robusto
DO $$
BEGIN
  BEGIN
    EXECUTE 'WITH ranked AS (
      SELECT id, qr_code_id, user_id,
             row_number() over (partition by qr_code_id, user_id order by created_at desc, id desc) as rn
      FROM public.qr_redemption_logs)
    DELETE FROM public.qr_redemption_logs WHERE id IN (SELECT id FROM ranked WHERE rn > 1)';
  EXCEPTION WHEN undefined_column THEN
    EXECUTE 'WITH ranked AS (
      SELECT id, qr_code_id, user_id,
             row_number() over (partition by qr_code_id, user_id order by id desc) as rn
      FROM public.qr_redemption_logs)
    DELETE FROM public.qr_redemption_logs WHERE id IN (SELECT id FROM ranked WHERE rn > 1)';
  END;
END $$;

-- 7) Unique index idempotente
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname='public' AND tablename='qr_redemption_logs' AND indexname='uniq_qrrl_qrcode_user'
  ) THEN
