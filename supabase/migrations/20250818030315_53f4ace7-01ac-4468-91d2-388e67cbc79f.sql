...
-- Add security logging trigger for pre-registration access
CREATE OR REPLACE FUNCTION log_pre_registration_access()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.admin_logs (
    ...
    'pre_registration_accessed',
    auth.uid(),
    'User accessed pre-registration data: ' || NEW.email,
    'security_audit'
  );
  RETURN NEW;
END;
...
CREATE TRIGGER log_pre_registration_access_trigger
  AFTER SELECT ON public.pre_registered_users
  FOR EACH ROW
  EXECUTE FUNCTION log_pre_registration_access();