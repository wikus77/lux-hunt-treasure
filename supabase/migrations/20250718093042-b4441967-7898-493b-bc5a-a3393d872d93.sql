...
CREATE TABLE IF NOT EXISTS public.user_mission_status (
  ...
  mission_progress_percent REAL GENERATED ALWAYS AS (
    clues_found::float / GREATEST(total_clues, 1) * 100
  ) STORED,
  mission_started_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  mission_days_total INTEGER DEFAULT 30,
  mission_days_remaining INTEGER GENERATED ALWAYS AS (
    GREATEST(mission_days_total - DATE_PART('day', NOW() - mission_started_at)::int, 0)
  ) STORED,
  mission_status TEXT DEFAULT 'ATTIVA',
  prize_city TEXT,
  prize_street TEXT,
  prize_coordinates GEOGRAPHY(POINT, 4326),
  prize_name TEXT,
