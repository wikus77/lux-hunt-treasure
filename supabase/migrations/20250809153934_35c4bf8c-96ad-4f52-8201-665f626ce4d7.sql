-- Align qr_redeem to use public.qr_codes and public.qr_redemption_logs
-- Add helpful indexes (no-op if they already exist)
...
-- Create or replace function with Haversine and proper logging
...
DECLARE
  ...
  v_max_m int := 100; -- default threshold since qr_codes may not have max_distance_meters
...
BEGIN
  ...
  IF p_lat IS NOT NULL AND p_lng IS NOT NULL AND qc.lat IS NOT NULL AND qc.lng IS NOT NULL THEN
    v_dist_m := 2 * 6371000 * asin(
      sqrt(
        pow(sin(radians((p_lat - qc.lat) / 2)), 2) +
        cos(radians(qc.lat)) * cos(radians(p_lat)) *
        pow(sin(radians((p_lng - qc.lng) / 2)), 2)
      )
    );

    IF v_dist_m > v_max_m THEN
      -- Optional: log out_of_range without consuming redemption
      INSERT INTO public.qr_redemption_logs (qr_code_id, user_id, user_lat, user_lng, distance_meters, success, failure_reason)
      VALUES (qc.id, v_uid, p_lat, p_lng, v_dist_m, false, 'out_of_range');
  ...
  -- Idempotency: already redeemed successfully by this user
  ...
  -- Redemption OK: insert success log
  ...
  VALUES (qc.id, v_uid, p_lat, p_lng, v_dist_m, true, NULL, jsonb_build_object('reward_type', qc.reward_type));
