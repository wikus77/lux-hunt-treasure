-- ========== EMERGENCY FIX: QR MAP + REDEEM (FIXED) ==========
begin;

-- 0) Update existing app_config or create with correct structure
DO $$
BEGIN
  -- Check if app_config exists and has the old structure
  IF EXISTS (SELECT 1 FROM information_schema.tables 
             WHERE table_schema = 'public' AND table_name = 'app_config') THEN
    
    -- Add value_jsonb column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_schema = 'public' AND table_name = 'app_config' AND column_name = 'value_jsonb') THEN
      ALTER TABLE public.app_config ADD COLUMN value_jsonb jsonb;
    END IF;
...
insert into public.app_config (key, value_int, value_jsonb)
values ('marker_min_zoom', 17, jsonb_build_object('minZoom', 17))
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables 
             WHERE table_schema = 'public' AND table_name = 'qr_redemption_logs') THEN
