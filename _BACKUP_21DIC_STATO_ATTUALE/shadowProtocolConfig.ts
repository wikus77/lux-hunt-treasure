...
export const SHADOW_PROTOCOL_TIMING = {
  ...
  // 🔧 REDUCED 50%: Was 90s/300s/8 events — Now 180s/600s/4 events
  MIN_INTERVAL_MS: 180_000,   // 3 minuti (was 1.5)
  MAX_INTERVAL_MS: 600_000,   // 10 minuti (was 5)
  // Limite massimo eventi per sessione (anti-spam)
  MAX_EVENTS_PER_SESSION: 4,  // (was 8)
...
/**
 * EVOLUTION LEVEL DESCRIPTIONS
 * 
 * SHADOW:
 *   0 = Observer - "We see your movement."
 *   1 = Interfering - "We adjust your frequency."
 *   2 = Hostile - "Stop repeating that pattern."
 *   3 = Invasive - "We mapped you. This is your last warning."
 * 
 * MCP:
 *   0 = Neutral - Standard briefings
 *   1 = Supportive - Encouraging messages
 *   2 = Protective - Countering Shadow
 *   3 = Critical - Urging behavior change
 * 
 * ECHO:
 *   0 = Background - Subtle whispers
 *   1 = Anxious - Worried warnings
 *   2 = Paranoid - Suspicious hints
 *   3 = Cryptic - Fragmented messages
 */

/**
 * EVOLUTION_TEXT_MODIFIERS - Text fragments to add based on evolution level
 * Used to modify template text dynamically
 */
export const EVOLUTION_TEXT_MODIFIERS = {
  SHADOW: {
    0: '', // Observer - no modifier
...
/**
 * getEvolutionModifiedText - Add evolution-based modifier to template text
 */
export function getEvolutionModifiedText(
  text: string,
  entity: ShadowEntity,
...
// ============================================================================
// 🆕 v8: TIME-OF-DAY VARIANTS
// ============================================================================

/**
 * isNightWindow - Check if current time is in the "night window" (02:00-05:00)
 * During this window, SHADOW is more active and messages are more intense
 */
export function isNightWindow(): boolean {
  const hour = new Date().getHours();
  return hour >= 2 && hour < 5;
}

/**
 * getTimeOfDayWeightModifier - Get weight modifier based on time of day
 * Night window: SHADOW +50%, MCP -30%, ECHO +20%
 */
export function getTimeOfDayWeightModifier(entity: ShadowEntity): number {
  if (!isNightWindow()) return 1.0;
  
  switch (entity) {
    case 'SHADOW': return 1.5;
    case 'MCP': return 0.7;
    case 'ECHO': return 1.2;
  }
}

// ============================================================================
// 🆕 v8: RARE STORY TEMPLATES
// These templates expand the M1SSION universe and trigger rarely
// ============================================================================

export const RARE_STORY_TEMPLATES: ShadowMessageTemplate[] = [
  {
    id: 'rare_shadow_not_first',
    entity: 'SHADOW',
    intensity: 3,
    weekRange: [1, 4],
    blocking: true,
    allowOnAllProtectedRoutes: true,
    weight: 0, // Selected manually, not by weight
    text: 'You are not the first.\n\nJust the one we have not erased yet.',
  },
  {
    id: 'rare_shadow_mcp_secret',
    entity: 'SHADOW',
    intensity: 2,
    weekRange: [2, 4],
    blocking: true,
    allowOnAllProtectedRoutes: true,
    weight: 0,
    text: 'MCP hides more than they admit.\n\nAsk them about 14.12.',
  },
  {
    id: 'rare_shadow_echo_origin',
    entity: 'SHADOW',
    intensity: 2,
    weekRange: [2, 4],
    blocking: true,
    allowOnAllProtectedRoutes: true,
    weight: 0,
    text: 'ECHO was once one of us.\n\nNow they speak in fragments, unable to choose.',
  },
  {
    id: 'rare_mcp_shadow_limits',
    entity: 'MCP',
    intensity: 2,
    weekRange: [1, 4],
    blocking: true,
    allowOnAllProtectedRoutes: true,
    weight: 0,
    text: 'Shadow has limits.\n\nYou are not one of them.',
  },
  {
    id: 'rare_mcp_other_agents',
    entity: 'MCP',
    intensity: 2,
    weekRange: [2, 4],
    blocking: true,
    allowOnAllProtectedRoutes: true,
    weight: 0,
    text: 'Other agents have walked this path.\n\nSome found treasures. Some found answers. Some... did not return.',
  },
  {
    id: 'rare_echo_vanished',
    entity: 'ECHO',
    intensity: 2,
    weekRange: [1, 4],
    blocking: true,
    allowOnAllProtectedRoutes: true,
    weight: 0,
    text: 'we saw someone else on this channel.\n\nthey vanished.',
  },
  {
    id: 'rare_echo_remember',
    entity: 'ECHO',
    intensity: 2,
    weekRange: [2, 4],
    blocking: true,
  ...
  },
  {
    ...
    allowOnAllProtectedRoutes: true,
    weight: 0,
    text: 't̷h̷e̷ ̷t̷r̷e̷a̷s̷u̷r̷e̷ ̷i̷s̷ ̷n̷o̷t̷ ̷w̷h̷a̷t̷ ̷y̷o̷u̷ ̷t̷h̷i̷n̷k̷',
  },
];

/**
 * RARE_TEMPLATE_CHANCE - Probability (0-1) to show a rare template when conditions are met
 */
export const RARE_TEMPLATE_CHANCE = 0.08; // 8% chance

/**
 * shouldTriggerRareTemplate - Check if conditions are met for a rare template
 * Conditions: high threat OR night time OR many overlays seen
 */
export function shouldTriggerRareTemplate(
  threatLevel: number,
  overlaysSeenThisSession: number,
  rareConsumedCount: number
): boolean {
  // Limit total rare templates per session
  if (rareConsumedCount >= 2) return false;
  
  // Need at least 3 overlays before rare can appear
  if (overlaysSeenThisSession < 3) return false;
  
  const isNight = isNightWindow();
  const isHighThreat = threatLevel >= 4;
  const manyOverlays = overlaysSeenThisSession >= 5;
  
  // One of the conditions must be met
  if (!isNight && !isHighThreat && !manyOverlays) return false;
  
  // Random chance
  return Math.random() < RARE_TEMPLATE_CHANCE;
}

/**
 * pickRareTemplate - Select a random rare template that hasn't been consumed
 */
export function pickRareTemplate(
  consumedIds: string[],
  phase: MissionPhase
): ShadowMessageTemplate | null {
  const eligible = RARE_STORY_TEMPLATES.filter(t => {
    if (consumedIds.includes(t.id)) return false;
...
// ============================================================================
// 🆕 v8: BRANCHING REACTION TEMPLATES
// Special templates triggered by specific user behaviors
// ============================================================================

export const BRANCHING_TEMPLATES: ShadowMessageTemplate[] = [
  // User ignored SHADOW warning and opened map anyway
  {
    id: 'branch_shadow_ignored',
    entity: 'SHADOW',
    intensity: 3,
    weekRange: [0, 4],
    blocking: true,
    allowOnAllProtectedRoutes: true,
    weight: 0,
    text: 'You ignored our warning.\n\nWe see everything, Echo.\n\nThis defiance is noted.',
  },
  // User dismissed overlays too fast multiple times
  {
    id: 'branch_shadow_fast_dismiss',
    entity: 'SHADOW',
    intensity: 2,
    weekRange: [0, 4],
    blocking: true,
    allowOnAllProtectedRoutes: true,
    weight: 0,
    text: 'You are not listening.\n\nWe will find other ways.',
  },
  // User followed MCP hint (went to /intelligence after prompt)
  {
    id: 'branch_mcp_followed',
    entity: 'MCP',
    intensity: 1,
    weekRange: [0, 4],
    blocking: false,
    allowOnAllProtectedRoutes: true,
    weight: 0,
    text: 'Good. You listened.\n\nStay vigilant, Agent.',
  },
  // User followed MCP hint to check intelligence
  {
    id: 'branch_mcp_intel_checked',
    entity: 'MCP',
    intensity: 1,
    weekRange: [0, 4],
    blocking: false,
    allowOnAllProtectedRoutes: true,
    weight: 0,
    text: 'Intelligence accessed.\n\nYou understand the importance of information.\n\nProceeding optimally.',
  },
];

/**
 * getBranchingTemplate - Get a branching template based on user behavior
 */
export function getBranchingTemplate(
  behaviorType: 'ignored_warning' | 'fast_dismiss' | 'followed_hint' | 'intel_checked'
): ShadowMessageTemplate | null {
  switch (behaviorType) {
    case 'ignored_warning':
      return BRANCHING_TEMPLATES.find(t => t.id === 'branch_shadow_ignored') || null;
    case 'fast_dismiss':
      return BRANCHING_TEMPLATES.find(t => t.id === 'branch_shadow_fast_dismiss') || null;
    case 'followed_hint':
      return BRANCHING_TEMPLATES.find(t => t.id === 'branch_mcp_followed') || null;
    case 'intel_checked':
      return BRANCHING_TEMPLATES.find(t => t.id === 'branch_mcp_intel_checked') || null;
    default:
      return null;
  }
}
