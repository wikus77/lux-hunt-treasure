...
import { 
  ...
  lockPopupQueue, 
  unlockPopupQueue,
  POPUP_QUEUE_UNLOCKED_EVENT 
} from '@/utils/popupQueue';

const STORAGE_KEY = 'm1ssion_reward_popup_dismissed';
const POPUP_ID = 'reward-zone-popup';
...
export const RewardZonePopup: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Function to attempt showing popup (respects queue)
    const attemptShow = () => {
      if (!canShowPopup(POPUP_ID)) {
        console.log('[RewardZonePopup] ⏳ Queue busy, waiting for unlock...');
        return false;
      }
      console.log('[RewardZonePopup] ✅ Showing popup NOW!');
      lockPopupQueue(POPUP_ID);
      setIsVisible(true);
      return true;
    };

    // Show popup after 1 minute
    console.log('[RewardZonePopup] ⏰ Popup will attempt to show in 1 minute...');
    const timer = setTimeout(() => {
      if (!attemptShow()) {
        // Queue was busy, listen for unlock
        const handleQueueUnlock = () => {
          // Small delay to let other pending popups settle
          setTimeout(() => {
        ...
        window.addEventListener(POPUP_QUEUE_UNLOCKED_EVENT, handleQueueUnlock, { once: true });
      }
  ...
  const executeFlytTo = useCallback((marker: MarkerData) => {
    ...
    setIsVisible(false);
    unlockPopupQueue(POPUP_ID); // 🔓 Release queue lock
