...
export const BriefingFlipOverlay: React.FC<BriefingFlipOverlayProps> = ({
  ...
  const handleDismissForever = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    if (import.meta.env.DEV) console.log('[VIDEO DEBUG] dismiss click', storageKey);
    safeSet(storageKey, 'true');
    if (import.meta.env.DEV) console.log('[VIDEO DEBUG] after set', storageKey, typeof localStorage !== 'undefined' ? localStorage.getItem(storageKey) : null, 'href', typeof location !== 'undefined' ? location.href : '');
