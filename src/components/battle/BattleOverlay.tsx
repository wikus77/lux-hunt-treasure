...
export function BattleOverlay({
  ...
  const [showLaunchButton, setShowLaunchButton] = useState(false); // 🆕 Show button after countdown

  // Reset countdown and button state when activated
  useEffect(() => {
    if (isActive) {
      setCountdown(10);
  ...
  useEffect(() => {
    ...
    if (countdown > 0) {
      ...
      // 🆕 Instead of auto-closing, show the launch button
      setShowLaunchButton(true);
    }
  }, [isActive, countdown]);

  // 🆕 Handle manual launch (activates audio from user tap)
  const handleLaunchClick = () => {
    // Play epic launch sound (user tap enables audio)
    playBeep(400, 100, 0.5);
    setTimeout(() => playBeep(600, 100, 0.5), 100);
    setTimeout(() => playBeep(800, 100, 0.5), 200);
    setTimeout(() => playBeep(1000, 150, 0.6), 300);
  ...
  return (
    ...
    >
      ...
      {/* Center - Countdown OR Launch Button */}
