...
// 🎬 Shop Overlay - iOS WKWebView OPTIMIZED (identico agli altri)
import React, { useEffect, useState, useCallback, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { createPortal } from 'react-dom';

interface ShopFlipOverlayProps {
  open: boolean;
...
export const ShopFlipOverlay: React.FC<ShopFlipOverlayProps> = ({
  open,
  originRect,
  onClose,
  children
  ...
  // Create portal
  useEffect(() => {
    let container = document.getElementById('m1-shop-portal');
    if (!container) {
      container = document.createElement('div');
      container.id = 'm1-shop-portal';
  ...
  const getTransformOrigin = () => {
    ...
    if (!rect) return '50% 10%'; // Default: top-center for pill
...
export default ShopFlipOverlay;
