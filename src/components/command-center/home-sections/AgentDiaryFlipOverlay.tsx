...
// 🎬 Agent Diary Overlay - iOS WKWebView OPTIMIZED (identico agli altri)
import React, { useEffect, useState, useCallback, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { createPortal } from 'react-dom';

interface AgentDiaryFlipOverlayProps {
  open: boolean;
...
export const AgentDiaryFlipOverlay: React.FC<AgentDiaryFlipOverlayProps> = ({
  open,
  originRect,
  onClose,
  children
  ...
  // Create portal
  useEffect(() => {
    let container = document.getElementById('m1-agentdiary-portal');
    if (!container) {
      container = document.createElement('div');
      container.id = 'm1-agentdiary-portal';
  ...
  const getTransformOrigin = () => {
    ...
    if (!rect) return '50% 30%'; // Default: center-top
...
export default AgentDiaryFlipOverlay;
