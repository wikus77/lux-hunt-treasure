...
/**
 * HOME INTRO VIDEO - Video briefing che parte all'ingresso nella Home
 * 
 * Features:
 ...
 * - Dissolvenza alla fine verso la Home
 */

import React, { useState, useEffect, useCallback, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { VolumeX, Volume2 } from 'lucide-react';
import './HomeIntroVideo.css';

const VIDEO_SRC = '/assets/video/HOME-BRIF-VIDEO.mp4';
const STORAGE_KEY = 'm1_home_video_dismissed';
...
const HomeIntroVideo: React.FC<HomeIntroVideoProps> = ({ onComplete, userEmail }) => {
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      ...
      console.log('[HomeIntroVideo] 🔧 Admin detected, video reset');
  ...
  const handleVideoError = useCallback(() => {
    console.warn('[HomeIntroVideo] Video failed to load');
    setVideoError(true);
    setIsVisible(false);
    onComplete();
  }, [onComplete]);
  ...
  return (
    <AnimatePresence>
      <motion.div
        className={`home-intro-video-container ${isFading ? 'fading' : ''}`}
        initial={{ opacity: 1 }}
      ...
      >
        ...
        {!audioEnabled && (
          <motion.div 
            className="home-intro-audio-hint"
            initial={{ opacity: 0 }}
            animate={{ opacity: 0.6 }}
        ...
        <div className="home-intro-controls">
          {/* Tasto "Non mostrare più" */}
          <motion.button
            className="home-intro-dismiss-btn"
            onClick={(e) => {
              e.stopPropagation();
