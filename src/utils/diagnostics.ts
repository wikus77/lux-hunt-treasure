...
const dryRunPreferences = async (userId?: string): Promise<any> => {
  try {
    ...
    const url = new URL(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/notifier-engine/dry-run`);
...
export const initDiagnostics = () => {
  ...
  (window as any).__M1_NOTIF_TEST__ = {
    ...
    async dryRunPref(userId: string, max=5, cooldownHours?: number) {
      ...
      const baseUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/notifier-engine`;
