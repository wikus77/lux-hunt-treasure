    ...
    try {
      ...
      if (error) {
        ...
        Sentry.captureException(error, {
          tags: { function: 'handle-buzz-press' },
          contexts: { request: { payload } }
        });
      ...
      if (!data || !data.success) {
        ...
        const errorMessage = data?.errorMessage || data?.error || "Errore durante l'elaborazione dell'indizio";
        
        // Track business logic errors
        Sentry.addBreadcrumb({
          message: 'Buzz API returned failure',
      ...
      // Track successful buzz events
      Sentry.addBreadcrumb({
        message: 'Buzz API successful',
        data: { generateMap, cost: data.buzz_cost },
        level: 'info'
      });
