    ...
    try {
      ...
      if (refreshToken && accessToken) {
        ...
        if (!sessionData.error && sessionData.data?.session) {
          ...
          localStorage.setItem(`sb-${(import.meta.env.VITE_SUPABASE_REF || process.env.VITE_SUPABASE_REF || 'project')}-auth-token`, JSON.stringify(sessionBackup));
  ...
  useEffect(() => {
    const initializeSession = async () => {
      ...
      try {
        ...
        const storedSession = localStorage.getItem(`sb-${(import.meta.env.VITE_SUPABASE_REF || process.env.VITE_SUPABASE_REF || 'project')}-auth-token`);
        if (storedSession) {
          try {
