        ...
        <div className="bg-white/5 rounded-lg p-4 mb-6">
          ...
          {stats ? (
            <div className="grid grid-cols-3 gap-4 text-sm">
              {[10, 30, 50].map((tier) => {
                ...
                return (
                  <div key={tier} className="bg-black/30 rounded-lg p-3">
                    <p className="font-bold text-lg">{tier} M1U</p>
                    <p className="text-white/50">Available: <span className="text-white">{tierData?.available ?? '?'}</span></p>
          ...
          {stats && (
            <div className="mt-4 pt-4 border-t border-white/10 grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-white/50">Purchases today:</span>
                <p className="font-bold">{stats.user_purchases_today ?? 0} / 10</p>
              </div>
              <div>
                <span className="text-white/50">Total M1U won:</span>
                <p className="font-bold text-yellow-400">{stats.user_total_wins?.toLocaleString() ?? 0}</p>
              </div>
            </div>
          )}
        </div>
              ...
              return (
                ...
                >
                  {isPurchasing === tier ? (
                    ...
                    <>
                      <CreditCard className={`w-8 h-8 mx-auto mb-2 ${canAfford ? 'text-yellow-400' : 'text-gray-500'}`} />
