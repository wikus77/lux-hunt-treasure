...
export default function QRRedeemPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const base = (import.meta as any).env?.VITE_QR_VALIDATE_URL as string | undefined || `${supabase.functions.url}/validate-qr`;
        const res = await fetch(`${base}?c=${encodeURIComponent(code)}`, { method: 'GET' });
        if (!res.ok) throw new Error('validate_failed');
        const j = await res.json();
        if (import.meta.env.DEV) console.debug('[QR] validate resp:', j);
        if (j?.valid) {
          setRow({
            code,
