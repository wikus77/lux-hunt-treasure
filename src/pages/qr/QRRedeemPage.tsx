...
type V = {
  ...
  code?: string;
  title?: string | null;
  reward?: { type: string; buzz?: number } | null;
  message?: string | null;
};
...
export default function QRRedeemPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (r.ok && (r as any).body?.valid) setV((r as any).body as V);
      else if (r.status === 404 || (r.ok && (r as any).body?.valid === false)) {
        setV({ valid: false });
  ...
  const redeem = async () => {
    if (!hasSession) {
      window.location.href = `/login?redirect=${encodeURIComponent('/qr/' + code)}`;
      return;
    }
    const r = await redeemQR(code);
    if (r.ok) toast.success('Riscatto completato');
    else {
      const msg = String((r as any).body?.error || '');
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center">
      <div className="max-w-md w-full p-6 text-center border rounded-xl bg-card">
        ...
        <Button className="w-full" onClick={redeem}>
          🎯 Riscatta
        </Button>
