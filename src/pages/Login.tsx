...
// Solution: Hide SSO buttons on iOS native until OAuth flow is fixed
// 
// TO RE-ENABLE: Set this to false or remove the flag entirely
// Code paths are preserved, only UI visibility is affected
// ============================================================================
const IOS_SSO_TEMPORARILY_DISABLED = true;

// Check if SSO should be shown (hidden on iOS native when flag is true)
function shouldShowSSOButtons(): boolean {
  if (!IOS_SSO_TEMPORARILY_DISABLED) return true;
  
  // Only hide on iOS native Capacitor app
...
const Login = () => {
  ...
  const renderSignUpScreen = () => (
    ...
    >
      ...
      {/* Provider Buttons - CONDITIONALLY RENDERED (iOS native fix) */}
      {shouldShowSSOButtons() && (
        <>
          ...
          {/* OR Divider - only shown when SSO buttons are visible */}
