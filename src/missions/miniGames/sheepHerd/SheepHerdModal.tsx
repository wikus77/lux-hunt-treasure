/**
 * Sheep Herd (Daily) — server-real, 120s rounds, two-phase daily flow.
...
import { claimDailyPhase, MISSION_ID_SHEEP_HERD_V1 } from '@/missions/serverReal/claimDailyPhase';
import {
  startMission,
  completePhase1,
  completePhase2,
...
import { parseSheepHerdParams } from './sheepHerdProgression';
import { SheepHerdGameCanvas } from './SheepHerdGameCanvas';
...
export const SheepHerdModal: React.FC<ShellGameProps> = ({ onClose, onComplete }) => {
  const { t } = useTranslation();
  const [step, setStep] = useState<Step>('loading');
  const [params, setParams] = useState<SheepHerdGameParams | null>(null);
  ...
  const tryPhase2ThenPhase1 = useCallback(async () => {
    setErrorMsg('');
    const res2 = await claimDailyPhase('start_phase2', MISSION_ID_SHEEP_HERD_V1);
    const p2 = parseSheepHerdParams(res2.progress);
    if (res2.ok && res2.phase === 2 && p2) {
      startMission(MISSION_ID_SHEEP_HERD_V1);
      setParams(p2);
      setActivePhase(2);
      setRoundKey((k) => k + 1);
      setStep('playing_p2');
    ...
    const res1 = await claimDailyPhase('start_phase1', MISSION_ID_SHEEP_HERD_V1);
    if (!res1.ok) {
      setErrorMsg(res1.error || t('daily_sheep.error_network'));
      setStep('error');
      return;
    }

    const p1 = parseSheepHerdParams(res1.progress);
    ...
    if (!p1) {
      setErrorMsg(t('daily_sheep.error_network'));
      setStep('error');
      return;
    }
  ...
  const submitRound = useCallback(
    async (payload: RoundOutcomePayload, phase: 1 | 2) => {
      setSubmitting(true);
      const action = phase === 1 ? 'complete_phase1' : 'complete_phase2';
      const res = await claimDailyPhase(action, MISSION_ID_SHEEP_HERD_V1, payload as unknown as Record<string, unknown>);
      setSubmitting(false);

      if (!res.ok) {
        setErrorMsg(res.error || t('daily_sheep.error_network'));
        setStep('error');
        return;
      }
    ...
    <div style={fullScreenStyle}>
      ...
      <AnimatePresence mode="wait">
        ...
        {(step === 'playing' || step === 'playing_p2') && params && (
          ...
          >
            ...
            <p style={{ color: 'rgba(0, 209, 255, 0.88)', fontSize: 13, margin: '0 0 8px', textAlign: 'center' }}>
              {t('daily_sheep.subtitle')}
            </p>
            <p style={{ color: 'rgba(255,255,255,0.72)', fontSize: 14, margin: '0 0 6px', textAlign: 'center', maxWidth: 400, marginLeft: 'auto', marginRight: 'auto' }}>
              {t('daily_sheep.instruction', {
                required: params.required_in_pen,
                maxLost: params.max_lost,
              })}
            </p>
            <p style={{ color: 'rgba(0, 255, 200, 0.75)', fontSize: 12, margin: '0 0 12px', textAlign: 'center', maxWidth: 400, marginLeft: 'auto', marginRight: 'auto' }}>
              {t('daily_sheep.hint_touch')}
            </p>
            <SheepHerdGameCanvas params={params} roundKey={roundKey} onRoundComplete={handleRoundComplete} />
          </motion.div>
        )}
        ...
        {step === 'fail_round' && (
          ...
          >
            <p style={{ color: '#ff6b6b', fontSize: 22, fontWeight: 700, textAlign: 'center' }}>{t('daily_sheep.fail')}</p>
            <button
              type="button"
              disabled={submitting}
              onClick={() => {
        ...
        {step === 'phase1_done' && (
          ...
          >
            <p style={{ color: '#00FF88', fontSize: 22, fontWeight: 700 }}>{t('daily_sheep.success')}</p>
        ...
        {step === 'phase2_win' && (
          <motion.div
            ...
            style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}
          >
            <p style={{ color: '#00FF88', fontSize: 24, fontWeight: 700 }}>{t('daily_sheep.success')}</p>
        ...
        {step === 'phase2_fail' && (
          <motion.div
            ...
            style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}
          >
            <p style={{ color: '#ff6b6b', fontSize: 22, fontWeight: 700 }}>{t('daily_sheep.fail')}</p>
        ...
        {step === 'error' && (
          <motion.div
            ...
            style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}
          >
            <p style={{ color: '#ff6b6b', fontSize: 16 }}>{errorMsg || t('daily_sheep.error_network')}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
